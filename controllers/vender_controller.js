...
const promotinAdd = async (req, res) => {
  try {
    const {
      venderId, cupon_code, cupon_title, numberOfcoupon,
      expire_date, start_date, discountType, id, appliesCoupon, appliesType,
      sameUser_discountLimit, discountAmount
    } = req.body;

    // Check for required parameters
    if (!venderId || !cupon_code || !cupon_title || !numberOfcoupon ||
        !expire_date || !start_date || !appliesCoupon || !sameUser_discountLimit) {
      return res.status(400).json({
        "result": "false",
        "message": "Required parameters are venderId, cupon_code,  cupon_title, numberOfcoupon, " +
...
const withdrawRequest=async(req,res)=>{
  try {
    const {venderId,amount,withdrawalType,bankAcc_name,bankAcc_number,swiftCode,mobile_money_number} = req.body;
    if (!venderId || !amount ||!withdrawalType) {
        return res.status(400).json({ "result": "false", "message": "required parameters are venderId,amount,withdrawalType,and optionals are bankAcc_name,bankAcc_number,swiftCode,mobile_money_number" });
    }
    ...
    const sales = await Order.find({shopId:venderId});
        ...
        const existDatas = await Withdraw.find({venderId,status: 1 }); 
        let withdraw = 0;
        if (existDatas && existDatas.length > 0) {
            withdraw = existDatas.reduce((total, withdraw) => total + withdraw.amount, 0);
        }
		
           const Wable=Number(totalSum -withdraw );
  
if(Wable>=amount){
  const otp=generate_otp();
        // const insertData=new Withdraw({venderId,amount,withdrawalType,mobile_money_number,bankAcc_name,bankAcc_number,swiftCode,status_type:1});
        // const data=await insertData.save();
        // res.status(200).json({
        //   "result": "true",
      ...
      //});
       ...
       const vender = await Vender.findOneAndUpdate({_id:venderId},{otp:otp},{new:true});
        res.status(200).json({
          "result": "true",
          "message": "Yor request sent successfully",
          otp:otp,
...
// withdraw  request
const withdrawRequest_verify=async(req,res)=>{
  try {
    const {venderId,amount,withdrawalType,otp,bankAcc_name,bankAcc_number,swiftCode,mobile_money_number} = req.body;
    if (!venderId || !amount ||!withdrawalType) {
        return res.status(400).json({ "result": "false", "message": "required parameters are venderId,amount,withdrawalType,otp,and optionals are bankAcc_name,bankAcc_number,swiftCode,mobile_money_number" });
    }
    
    const existData= await Vender.findOne({_id:venderId,otp});
    if(!existData){
      return res.status(400).json({
        "result": "false",
        ...
        res.status(200).json({
          "result": "true",
          "message": "Withdrawal request sent and waiting for approval",
          data: data
      });
  ...
  try {
    ...
    if (!vendorId ) {
      return res.status(400).json({ "result": "false", "message": "Required field is vendorId" })
        ...
        const existData = await Withdraw.find({venderId:vendorId,withdraw_status: 1 }); 
        let withdraw = 0;
...
const createQrcodes=async(req,res)=>{
  try {
    ...
    const validationData=await Qrcode.findOne({shopId,qrcode_name});
    if(validationData){
      return res.status(400).json({result:"false",message:"Qr name is allready exist"})
    }
