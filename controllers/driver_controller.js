...
const driver_login = async (req, res) => {
  try {
    ...
    if (!password || !key) {
      return res.status(400).json({ "result": "false", "message": "required parameters are password,key and sent email or phone in key parameter" });
    }

    // Check if the key is an email or phone number
    const isEmail = key.includes('@');
    const searchQuery = isEmail ? { email: key } : { phone: key };

    const Data = await Driver.findOne({ $or: [searchQuery] });
    if (!Data) {
      return res.status(400).json({ "result": "false", "message": "You are not registered" });
    }

    const matchData = await Driver.findOne({ $or: [searchQuery], status: 1 });
    if (!matchData) {
      return res.status(400).json({
        "result": "false",
