...
const adminWallet_details = async (req, res) => {
  ...
  const driverAdd=async(req,res)=>{
	try {
		const {fname,latitude,longitude,lname,phone,email,password,conform_password,dob,fcm_id,city} = req.body;
    
		if (!phone || !fcm_id) {
			return res.status(400).json({ "result": "false", "message": "required parameters are fname,latitude,longitude, lname,phone,email,password,conform_password,dob,fcm_id,city,frontId_iamge,backId_iamge,vehical_iamge,document" });
		}
