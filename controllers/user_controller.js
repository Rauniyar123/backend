...
const userSignup_api = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are first_name,last_name,mobile_number,password,conform_password,fcm_id and user_profile,friend_referral_code and email are optional",
        });
    }
    ...
    if (existUser) {
      const userData = await User.findOneAndUpdate({ mobile_number }, objects, {
        new: true,
      });
      return res
        .status(200)
      ...
      if (!data) {
        ...
        const dinesh = await User.findOne({
          user_referral_code: user_referral_codes,
        });
        const insertRefferal_money = new Refferal({
          friendId: users_data._id,
          amount: 10,
          userId: dinesh._id,
        });
    ...
    if (!password) {
      res
        ...
        .json({
          result: "false",
          message:
            "required parameters are password and any one parameter required is fcm_id in both them(mobile_number,email)",
        });
    } else {
      const Data = await User.findOne({
        $or: [{ mobile_number: mobile_number }, { email: mobile_number }],
      });
      if (!Data) {
        res
          .status(400)
    ...
    if (!choose_status) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are mobile_number and choose_status,email (1 for SMS and 2 for email)",
        });
    } else {
      if (choose_status == 1) {
        ...
        if (!matchData) {
          return res
            .status(400)
            .json({ result: "false", message: "Mobile number does not match" });
        } else {
          const otp = generate_otp();
        ...
        if (!email) {
          res
            .status(400)
            .json({ result: "false", message: "email is required" });
        } else {
          const matchDatas = await User.findOne({ email: email });
          if (!matchDatas) {
            res
              .status(400)
              .json({ result: "false", message: "Email is empty" });
          } else {
            const Otp = generate_otp();
            const updatedDatas = await User.findOneAndUpdate(
              { email },
              { $set: { otp: Otp } },
              { new: true }
            );
  ...
  try {
    ...
    if (!mobile_number || !otp) {
      res
        ...
        .json({
          result: "false",
          message: "required parameters are mobile_number,otp",
        });
    } else {
  ...
  if (!mobile_number) {
    res
      ...
      .json({
        result: "false",
        message:
          "required parameters mobile_number and any one required parameter in both them(email,mobile_number)",
      });
  }
  try {
    const data = await User.findOne({
    ...
    if (data) {
      ...
      res
        .status(200)
        .json({ result: "true", message: "Otp send sucessfully", data: Data });
    } else {
      res
        .status(400)
        .json({
          result: "false",
          message: "Please send correct email and mobile_number",
        });
    }
  } catch (err) {
    res.status(400).json({ result: "false", message: err.message });
...
const verifyEmail_andMobile = async (req, res) => {
  ...
  if (!key || !otp) {
    res
      .status(400)
      .json({ result: "false", message: "required parameter are otp, key" });
  }
  try {
    const data = await User.findOne({
      $or: [{ mobile_number: key }, { email: key }],
      otp: otp,
    });
    if (!data) {
      res.status(400).json({ result: "false", message: "Invalid otp" });
      ...
      res
        ...
        .json({
          result: "true",
          message: "Your otp verified sucessfully",
          data: data,
        });
    }
  } catch (err) {
...
const resetPassword = async (req, res) => {
  ...
  if (!key || !password) {
    res
      ...
      .json({
        result: "false",
        message: "required parameter are password, key",
      });
  }
  try {
    // Hash the password before saving it
    ...
    if (!userId) {
      ...
      if (matchData) {
        const data = {
          ...
          user_referral_code: matchData.user_referral_code,
        };
        res.status(200).json({
          result: "true",
        ...
        res
          .status(400)
          .json({ result: "false", message: "User does not found" });
      }
    }
  } catch (err) {
    res.status(400).json({ result: "false", message: err.message });
...
const banner_list = async (req, res) => {
  try {
    const matchData = await Banner.find({
      banner_type: "Innt header banner",
      active_status: 0,
    });
    if (matchData) {
      res.status(200).json({
      ...
      res
        .status(400)
        .json({ result: "false", message: " data  does not found" });
    }
  } catch (err) {
    res.status(400).json({ result: "false", message: err.message });
  }
...
const inntFooterbanner_list = async (req, res) => {
  try {
    const matchData = await Banner.find({
      banner_type: "Innt footer banner",
      active_status: 0,
    });
    if (matchData) {
      res.status(200).json({
...
const inntOutFooterbanner_list = async (req, res) => {
  try {
    const matchData = await Banner.find({
      banner_type: "InntOut footer banner",
      active_status: 0,
    });
    if (matchData) {
      res.status(200).json({
...
const inntOutHeaderbanner_list = async (req, res) => {
  try {
    const matchData = await Banner.find({
      banner_type: "InntOut header banner",
      active_status: 0,
    });
    if (matchData) {
      res.status(200).json({
    ...
    if (!userId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters is  userId and optional are place_type, state,village_name,pin_code,building_no,city_name,landmark,longitude,latitude",
        });
    }
    ...
    if (
      ...
      res
        ...
        .json({
          result: "false",
          message:
            "required parameters are place_type,state,village_name,longitude,latitude,pin_code,building_no,city_name,landmark,userId,addressId",
        });
    } else {
      const updatedData = {
        place_type,
      ...
      const data = await User_Address.findByIdAndUpdate(
      ...
      res
        ...
        .json({
          result: "true",
          message: "data updated sucussfully",
          data: data,
        });
    }
  } catch (err) {
  ...
  try {
    ...
    if (!userId || !addressId) {
      res
        ...
        .json({
          result: "false",
          message: "required parmaters are addressId,userId",
        });
    } else {
      const matchData = await User_Address.findOne({
        _id: addressId,
        userId: userId,
      });
      if (!matchData) {
        res
          .status(400)
          .json({ result: "false", message: "Address does not found" });
      } else {
        const data = await User_Address.findOneAndDelete({
          _id: addressId,
          userId: userId,
        });
        res
          .status(200)
          .json({ result: "false", message: "Addrress deleted sucessfully" });
      }
    }
  } catch (err) {
    res.status(400).json({ result: "false", message: err.message });
...
const Get_address = async (req, res) => {
  try {
    ...
    if (!userId) {
      res
        .status(400)
        .json({ result: "false", message: "required parmaters are userId" });
    } else {
      ...
      res
        .status(200)
        .json({ result: "true", message: "Addrress list are ", data: data });
    }
  } catch (err) {
    res.status(400).json({ result: "false", message: err.message });
  }
  ...
  try {
    ...
    if (!maincategoryId) {
      return res
        ...
        .json({
          result: "false",
          message: "require paramter ismaincategoryId",
        });
    }
    ...
    if (!data) {
      ...
      res
        .status(200)
        .json({ result: "true", message: "Category list are", data: data });
    }
  } catch (err) {
    console.log(err);
    res.status(400).json({ result: "false", message: err.message });
  ...
  try {
    ...
    if (!categoryId) {
      res
        ...
        .json({
          result: "false",
          message: "require parameters are categoryId",
        });
    } else {
...
const like_dislike_product_api = async (req, res) => {
  ...
  if (!productId || !userId) {
    return res
      ...
      .json({
        result: "false",
        message: "Required parameters are userId and productId",
      });
  }
  ...
  try {
    ...
    if (findData) {
      if (findData.like_status === 1) {
        ...
        res
          .status(200)
          .json({ result: "true", message: "Product disliked successfully" });
      } else {
        await Like.findOneAndUpdate(
          { userId: userId, productId },
          { $set: { like_status: 1 } },
        ...
        res
          .status(200)
          .json({ result: "true", message: "Product liked successfully" });
      }
    } else {
...
const likeProduct_listApi = async (req, res) => {
  ...
  if (!userId) {
    res
      .status(400)
      .json({ result: "false", message: "required parameter are userId" });
  }

  try {
    const data = await Like.find({ userId: userId, like_status: 1 }).populate(
      "productId"
    );
    if (!data) {
      ...
      res
        ...
        .json({
          result: "true",
          message: "Product liked data list are ",
          data: mergedData,
        });
    }
  } catch (err) {
...
const givingRating_api = async (req, res) => {
  ...
  if (!userId || !productId || !rating) {
    res
      ...
      .json({
        result: "false",
        message:
          "required parameters are userId,venderId,productId,rating and comment is optional",
      });
  }
  ...
  try {
    ...
    if (data) {
      ...
      const totalRating =
        rater && rater.length > 0
          ? rater.reduce((sum, rating) => sum + parseInt(rating.rating), 0)
          : 0;
      ...
      res
        .status(200)
        .json({ result: "true", message: "User added rating sucessfully" });
    }
  } catch (err) {
    console.log(err);
    res.status(400).json({ result: "false", message: err.message });
...
const userLogin_withSocicalMedia = async (req, res) => {
  try {
    const { mobile_number, fcm_id, socical_media_id, userName, email } =
      req.body;
    const { user_profile } = req.file ? req.file.filename : {};
    if (!socical_media_id) {
      res
        ...
        .json({
          result: "false",
          message:
            "required parameters are socical_media_id,fcm_id and optional parameters are (mobile_number,email and userName,user_profile)",
        });
    } else {
  ...
  try {
    ...
    if (!sub_subcategoryId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameter are sub_subcategoryId",
        });
    }
    const productList = await Product.find({
      sub_subcategoryId: sub_subcategoryId,
    });
...
//           result: "false",
//           message:
//             "required parameter is productId,shopId,lat,lon and userId optional",
//         });
//     }
...
//     const delivery__times =
//       Number(time) +
//       Number(coveruserandshop_distance_timing) +
//       Number(covermindistance_time);
...
//       const productList = await Product.find({ _id: productId }).populate(
//         "venderId"
//       );
...
//       const sales = matchData.reduce(
//         (total, order) =>
//           total +
//           order.products.filter((product) => product.order_status === 6).length,
//         0
//       );

//       //const reviews list
//       const pratingCount = await Rating.find({
//         productId: productId,
//       }).countDocuments();
//       const reviewsList = await Rating.find({ productId: productId }).populate(
//         "userId"
//       );
...
//       //const reviews list
//       const shopProductList = await Product.find({ venderId: shopId }).limit(
//         50
//       );
...
//             item.image5,
//             item.video,
//           ].filter((image) => typeof image === "string" && image !== null),
//           product_code: item.product_code,
...
const productDetails = async (req, res) => {
  try {
    ...
    if (!productId || !shopId) {
      return res.status(400).json({
        result: "false",
        message: "required parameter is productId, shopId, lat, lon and userId optional",
      });
    }

    // Fetch the shop and product data in parallel
    const [shop, productLists] = await Promise.all([
      Vender.findOne({ _id: shopId }),
      Product.findOne({ _id: productId }).populate("venderId")
    ]);

    if (!shop || !productLists) {
      return res.status(400).json({ result: "false", message: "Records not found" });
    }
    ...
    const deliveryBoys = await Driver.find();
    let minDistance = deliveryBoys.length > 0 ? Math.min(...deliveryBoys.map(boy => calculateDistance(storeLat, storeLon, 22.7196, 75.8577))) : 5;

    const mindist = minDistance.toFixed(1);
    const covermindistance_time = Number(mindist * 10);

    // Calculate delivery time
    const time = parseInt(productLists.productPreparation_time.replace(/\D+/g, ""), 10);
    const delivery__times = time + coveruserandshop_distance_timing + covermindistance_time;

    // Calculate the delivery time
    const now = new Date();
    now.setMinutes(now.getMinutes() + delivery__times);
    const formattedTimeDifference = `${Math.floor(delivery__times / 60)} hours ${delivery__times % 60} minutes`;

    // Calculate delivery charge
    const delivery_charge_value = Number(distance * 0.9).toFixed(2);

    // Fetch additional data in parallel
    const [ratersCount, productsCount, shopRatings, likeCount, views, orders, reviews] = await Promise.all([
      Shoprating.countDocuments({ shopId }),
      Product.countDocuments({ venderId: shopId }),
      Shoprating.find({ shopId }),
      Like.countDocuments({ productId, like_status: 1 }),
      Viewshops.countDocuments({ shopId }),
      Order.find({ shopId }),
      Rating.find({ productId }).populate("userId")
    ]);

    // Calculate total and average ratings
    const totalRating = shopRatings.reduce((sum, rating) => sum + parseInt(rating.rating, 10), 0);
    const average_ratings = ratersCount > 0 ? totalRating / ratersCount : 0;

    // Calculate total sales
    const sales = orders.reduce((total, order) => total + order.products.filter(product => product.order_status === 6).length, 0);

    // Format reviews list
    const reviews_list = reviews.map(item => ({
      rating: item.rating,
      comment: item.comment,
      user_namef: item.userId.first_name,
      user_namel: item.userId.last_name,
    ...
    // Fetch more products in parallel
    const moreproducts = (await Product.find({ venderId: shopId }).limit(50)).map(item => ({
      image: item.image1,
      productId: item._id,
      shopId: item.venderId,
    }));
    ...
    if (userId) {
      ...
      if (userCart && userCart.products.some(product => product.productId.toString() === productId.toString())) {
        cart = 1;
      }
    }
    ...
    const data = [{
      ...
      categoryId: productLists.categoryId,
      venderId: productLists.venderId._id,
      product_name: productLists.product_name,
      description: productLists.description,
      images: [productLists.image1, productLists.image2, productLists.image3, productLists.image4, productLists.image5, productLists.video].filter(image => typeof image === "string" && image !== null),
      product_code: productLists.product_code,
      productType: productLists.productType,
      productSize: productLists.productSize,
      productPreparation_time: productLists.productPreparation_time,
      mrp_price: productLists.unit_price,
      sale_price: productLists.sale_price,
      discount: productLists.discount[0]?.discount_value,
      discountType: productLists.discount[0]?.discount_type,
      product_details: productLists.product_details,
      brand_name: productLists.brand_name,
      like_status: userId ? likeCount : 0,
      rating: productLists.rating,
      total_rating: reviews.length,
      cart_status: cart,
      qty: 0,
      attributes: productLists.product_variation,
      quantity: productLists.quantity,
      warranty: productLists.warranty,
      note: productLists.note,
      returnType: productLists.returnType,
      stock: productLists.stock,
      Tax: productLists.Tax,
      product_weight: productLists.product_weight,
      deliveryType: productLists.deliveryType,
      minimum_order: productLists.minimum_order,
      shop_logo: productLists.venderId.shop_logo,
      shop_address: productLists.venderId.shop_address,
      shop_country: productLists.venderId.country,
      shop_city: productLists.venderId.city,
      shop_name: productLists.venderId.shop_name,
      total_product: productsCount,
      total_sale: sales,
      total_rater: ratersCount,
      average_rating: average_ratings,
      delivery_fee: delivery_charge_value,
      delivery__time: formattedTimeDifference,
      liked__time: likeCount,
      page__views: views,
      reviews_list: reviews_list,
      moreproducts: moreproducts,
    }];
...
const productDetails = async (req, res) => {
  try {
    ...
    // Fetch delivery boys data and find minimum distance
    const deliveryBoys = await Driver.find();
    const minDistance = deliveryBoys.length > 0 ? Math.min(...deliveryBoys.map(boy => calculateDistance(storeLat, storeLon, 22.7196, 75.8577))) : 5;
    const mindist = minDistance.toFixed(1);
    const covermindistance_time = Number(mindist * 10);

    // Calculate delivery time
    const preparationTime = parseInt(productLists.productPreparation_time.replace(/\D+/g, ""), 10);
    const delivery__times = preparationTime + coveruserandshop_distance_timing + covermindistance_time;
    const formattedTimeDifference = `${Math.floor(delivery__times / 60)} hours ${delivery__times % 60} minutes`;
  ...
  try {
    ...
    if (!subcategoryId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameter is subcategoryId",
        });
    }
    const findData = await Sub_subcategory.find({
      subcategoryId: subcategoryId,
    });
    if (!findData) {
      res
        .status(400)
        .json({ result: "false", message: "record does not found" });
    } else {
      res
        .status(200)
        .json({
          result: "true",
          message: "Sub_subcategory data get sucessfully",
          data: findData,
        });
    }
  } catch (err) {
...
const givingShoprating = async (req, res) => {
  ...
  if (!userId || !shopId || !rating) {
    return res
      ...
      .json({
        result: "false",
        message:
          "required parameters are userId,shopId,rating and comment is optional",
      });
  }
  ...
  try {
    ...
    if (data) {
      ...
      res
        .status(200)
        .json({ result: "true", message: "Review updated sucessfully" });
    } else {
      const insertData = new Shoprating({ userId, shopId, rating, comment });
      await insertData.save();
      res
        .status(200)
        .json({ result: "true", message: "Review added rating sucessfully" });
    }
  } catch (err) {
    console.log(err);
    res.status(400).json({ result: "false", message: err.message });
  ...
  try {
    ...
    if (data.deletedCount > 0) {
      res
        ...
        .json({
          result: "true",
          message: "Products liked by user deleted successfully",
        });
    } else {
      res
        .status(404)
        .json({
          result: "false",
          message:
            "No products found to delete for the given userId and productId",
        });
    }
  } catch (err) {
    console.log(err);
...
const Addcart = async (req, res) => {
  try {
    const { userId, productId, attributes, qty, status, sourceName } = req.body;
    if (!userId || !productId) {
      return res
        .status(400)
        .json({
          result: "false",
          message:
            "required parameters are userId and productId,attributes,qty,status,sourceName",
        });
    }
    ...
    if (req.body.status === "0") {
      ...
      if (!cart) {
        const cart_data = new Cart({
          userId: userId,
          status: status,
          products: [{ productId, attributes, qty, source: sourceName }],
        });
  ...
  try {
    ...
    if (!userId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameters are userId, status",
        });
    }
    ...
    if (
      !cart_data ||
      cart_data.length === 0 ||
      !cart_data[0].products ||
      cart_data[0].products.length === 0
    ) {
      return res.status(400).json({
        result: "false",
      ...
      const charge = await Charge.findOne({}) || 0;
      res.status(200).json({
        result: "true",
  ...
  try {
    ...
    if (!userId || !productId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameters are userId,productId,status",
        });
    }

    const matchData = await Cart.findOne({
      userId: userId,
      booking_status: 0,
      status: status,
    });
    if (matchData.products.length === 1) {
      ...
      res
        .status(200)
        .json({ result: "true", message: "cart item deleted successfully" });
    } else {
  ...
  try {
    ...
    if (!categoryId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameter are categoryId",
        });
    }
...
const inntOut_productDetails = async (req, res) => {
  try {
    ...
    deliveryBoys.forEach((boy) => {
      const latitude = boy.geo_location.coordinates[1];
      const longitude = boy.geo_location.coordinates[0];
      const distance = calculateDistance(
        storeLat,
        storeLon,
        latitude,
  ...
  try {
    ...
    if (!userId || !products || !status) {
      return res
        ...
        .json({
          result: false,
          message: "Required parameters are userId, products, and status",
        });
    }
    ...
    if (!Array.isArray(productData)) {
      return res
        .status(400)
        .json({ result: false, message: "Products should be an array" });
    }
    ...
    if (!cart) {
      ...
      newProducts.forEach((productId) => {
        const productToAdd = productDetails.find(
          (product) => product.productId === productId
        );
        if (productToAdd) {
  ...
  try {
    ...
    if (!userId || !productids) {
      return res
        ...
        .json({
          result: "false",
          message: "Required parameters are userId and productids,status",
        });
    }
    ...
    if (!cart || cart.length === 0) {
      return res
        .status(400)
        .json({ result: "false", message: "Cart not found for the user" });
    }
    ...
    if (updatedCart) {
      return res
        .status(200)
        .json({ result: "true", message: "Cart items removed successfully" });
    } else {
      return res
        .status(400)
        .json({ result: "false", message: "Cart items not found" });
    }
  } catch (error) {
    console.log(error.message);
    res.status(500).json({ result: "false", message: error.message });
...
const checkOut = async (req, res) => {
  try {
    const { userId, products, total_item, total_price, save_amount, status } =
      req.body;
    ...
    if (
      ...
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are userId and products,total_item,save_amount, total_price,sent status 1 for only inntout ",
        });
    }
    ...
    if (data) {
      await Checkout.findOneAndUpdate(
        { userId, check_status: 0, status: status },
        {
          products: products,
      ...
      res
        ...
        .json({
          result: "true",
          message: "Checkout data inserted successfully",
          data: savedCart,
        });
    }
  } catch (error) {
  ...
  try {
    ...
    if (!userId || !checkoutId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are userId,checkoutId,note,addressId,tax,offer,discount,gift_card, subtotal,total,payment_status,delivery_place,payType,paymentMode,grand_total",
        });
    }
    ...
    if (payType === "wallet") {
      ...
      if (checkMoney.wallet < grand_total) {
        return res
          .status(400)
          .json({ result: "false", message: "You have not sufficient blance" });
      }
      ...
      const insertedData = new Transjection({
        transjectionId: transId,
        userId: userId,
        amount: grand_total,
        payment_status: "Order",
        payment_type: "Shipped",
      });
    ...
    findData.products.forEach((product) => {
      ...
      const adminTaken_charge = Number(
        (product.total * commissions.commission) / 100
      ).toFixed(2);
    ...
    await Cart.findOneAndUpdate(
      { userId: userId, booking_status: 0, status: 0 },
      { booking_status: 1 },
      { new: true }
    );
    await Checkout.findOneAndUpdate(
      { userId: userId, check_status: 0, status: 0 },
      updatedData,
      { new: true }
    );
...
const orderList = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ result: "false", message: "Required parameters is userId" });
    }
    const data = await Order.find({ userId, status: 0 })
      .populate("shopId")
      .sort({ _id: -1 });
    if (data) {
      res
      ...
      res
        ...
        .json({
          result: "false",
          message: "Order does not available successfully",
          data: data,
        });
    }
  } catch (error) {
  ...
  try {
    ...
    if (!orderId || !productId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameters are orderId and productId",
        });
    }
    ...
    const checkdata = await Checkout.findOne({
      _id: matchData.checkoutId,
    }).populate("addressId");
    const address = {
    ...
    if (returnOrder) {
      if (returnOrder.return_status == "0") {
        return_status = 1;
      } else if (returnOrder.return_status == "1") {
        return_status = 2;
      } else {
        return_status = 3;
      }
    ...
    const filteredProduct = matchData.products.find(
      (product) => product.productId.toString() === productId
    );
    const rating = await Rating.findOne({ userId: user, productId });

    const leftProducts = matchData.products.filter(
      (product) => product.productId.toString() !== productId
    );
    if (!filteredProduct) {
      return res
        .status(400)
        .json({ result: "false", message: "product not found in order" });
    }
    ...
    res.status(200).json({
      result: "true",
      message:
        "Order details retrieved successfully for productId: " + productId,
      path: "http://103.104.74.215:3037/uploads/",
      data: responseData,
    });
  } catch (err) {
...
const likeMultipleProducts = async (req, res) => {
  ...
  if (!productIds || !userId) {
    return res
      ...
      .json({
        result: false,
        message: "Required parameters are userId and an array of productIds",
      });
  }
  ...
  try {
    ...
    if (existingProductIds.length > 0) {
      ...
      res
        .status(200)
        .json({ result: true, message: "Like status updated sucessfully" });
    } else {
  ...
  try {
    ...
    if (!userId || !products || !cartId) {
      return res
        ...
        .json({
          result: false,
          message: "Required parameters are userId, cartId, and products",
        });
    }
    ...
    let cart = await Cart.findOneAndUpdate(
      ...
      {
        $set: {
          "products.$[elem].attributes": productUpdates[0].update.attributes,
          "products.$[elem].qty": productUpdates[0].update.qty,
        },
      },
      {
        new: true,
        arrayFilters: [{ "elem.productId": productUpdates[0].productId }],
      }
    );
  ...
  try {
    ...
    if (!userId || !productId) {
      return res
        ...
        .json({
          result: "false",
          message: "Required parameters is userId,productId",
        });
    }
    ...
    if (matchData) {
      res
        .status(200)
        .json({ result: "true", message: "View updated successfully" });
    } else {
...
const suggested_ProductList = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ result: "false", message: "required paramter is userId" });
    }

    const productList = await Viewsproduct.find({ userId: userId }).populate(
      "productId"
    );
    if (productList || productList.length > 0) {
      res
  ...
  try {
    ...
    if (!checkoutId) {
      return res
        ...
        .json({
          result: "false",
          message: "required paramter is checkoutId,addressId",
        });
    }

    const productList = await Checkout.findOneAndUpdate(
      { _id: checkoutId },
      { addressId },
      { new: true }
    );
    if (productList) {
      res
        .status(200)
        .json({ result: "true", message: "Address updated sucessfully" });
    } else {
      res.status(400).json({ result: "false", message: "Records not found" });
    }
  } catch (err) {
...
const updateOrder_status = async (req, res) => {
  try {
    ...
    if (!orderId || !status) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required paramter is orderId ,productId,status(Note: cancel for 1,confirm 2,packing 3,shipped 4, not delivered 5,delivered 6,return 7)",
        });
    }
    ...
    if (updateOrder) {
      res
        .status(200)
        .json({ result: "true", message: "Order cancel sucessfully" });
    } else {
      res.status(400).json({ result: "false", message: "Records not found" });
    }
  } catch (err) {
...
const inntShop_list = async (req, res) => {
  try {
    ...
    if (!lon || !lat) {
      return res
        .status(400)
        .json({ result: "false", message: "required parameters are lat, lon" });
    }

    const cat = await Maincategory.findOne({
      maincategory_englishName: "Innt",
    });
    const id = cat._id;
    ...
    if (!shoplist || shoplist.length === 0) {
      return res
        .status(400)
        .json({ result: "false", message: "Shop list not found" });
    }
    ...
    for (const shop of shoplist) {
      ...
      const storeLat = shop.location.coordinates[1];
      const storeLon = shop.location.coordinates[0];
      const distance = calculateDistance(lat, lon, storeLat, storeLon).toFixed(
        1
      );
...
const inntShop_details = async (req, res) => {
  try {
    ...
    if (!shopId) {
      return res
        .status(400)
        .json({ result: "false", message: "required parameters are shopId" });
    }
    ...
    if (!shopId) {
      return res
        .status(400)
        .json({ result: "false", message: "shopId do not found" });
    }
    ...
    const shopdata = {
      ...
      lon: shopData.location.coordinates[0],
    };
    ...
    res
      ...
      .json({
        result: "true",
        message: "Shop details list retrieved successfully",
        data: data,
      });
  } catch (err) {
    console.log(err.message);
...
const filterShopProduct = async (req, res) => {
  try {
    ...
    if (!shopId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are shopId and optional are (price, discount, rating)",
        });
    }
    ...
    if (!productData || productData.length === 0) {
      return res
        .status(400)
        .json({ result: "false", message: "Product data not found" });
    }
...
const favouriteShop = async (req, res) => {
  ...
  if (!shopId || !userId) {
    return res
      ...
      .json({
        result: "false",
        message: "Required parameters are userId and shopId",
      });
  }
  ...
  try {
    ...
    if (findData) {
      return res
        ...
        .json({
          result: "true",
          message: "Allready added in shop favourite list ",
        });
    } else {
      // If no record is found, insert a new one with like_status as 1
      const insertData = new Shopfavourite({
        userId,
        shopId,
        favourite_status: 1,
      });
      ...
      res
        .status(200)
        .json({ result: "true", message: "Shop favourite added successfully" });
    }
  } catch (err) {
    console.error(err);
    res.status(500).json({ result: "false", message: err.message });
...
const removefavouriteShop = async (req, res) => {
  ...
  try {
    ...
    if (findData) {
      ...
      res
        .status(200)
        .json({ result: "true", message: "Favourite deleted successfully" });
    }
  } catch (err) {
    console.error(err);
    res.status(500).json({ result: "false", message: err.message });
...
const favouriteShopList = async (req, res) => {
  ...
  if (!userId || !lat) {
    return res
      ...
      .json({
        result: "false",
        message: "required parameter are userId,lat,lon",
      });
  }

  try {
    const shoplist = await Shopfavourite.find({
      userId: userId,
      favourite_status: 1,
    }).populate("shopId");
    if (!shoplist) {
      ...
      for (const shop of shoplist) {
        ...
        const sales = matchData.reduce(
          (total, order) =>
            total +
            order.products.filter((product) => product.order_status === 6)
              .length,
          0
        );
...
const trendingNow = async (req, res) => {
  try {
    ...
    const matchData = await Order.find({
      updatedAt: { $gte: oneMonthAgo, $lte: today },
      "products.productId": { $in: id },
    });
    ...
    // Find product list
    const productList = await Product.find({ _id: { $in: productIds } }).limit(
      30
    );
    ...
    const data = mergedProductList.map((item) => ({
      ...
      discount:
        item.discount && item.discount.length > 0
          ? item.discount[0].discount_value
          : 0,
      discount_name:
        item.discount && item.discount.length > 0
          ? item.discount[0].discount_type
          : null,
    }));
    ...
    if (!userId || !productId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are userId and productId,attributes,qty ",
        });
    }
...
const InnoutcartList = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ result: "false", message: "required parameters are userId" });
    }

    const cart_data = await Inntoutcart.find({
      userId: userId,
      booking_status: 0,
    }).populate({
      path: "products.productId",
  ...
  try {
    ...
    if (!userId || !productId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameters are userId,productId",
        });
    }

    const matchData = await Inntoutcart.findOne({
      userId: userId,
      booking_status: 0,
    });
    if (matchData.products.length === 1) {
      await Inntoutcart.findOneAndDelete({
        userId: userId,
        products: { $elemMatch: { productId: productId } },
        booking_status: 0,
      });
...
const updatePhone = async (req, res) => {
  try {
    ...
    const matchDatas = await User.findOne({
      _id: userId,
      mobile_number: phone,
    });
    if (matchDatas) {
...
const updateEmail = async (req, res) => {
  try {
    ...
    if (updatedVendor) {
      ...
      return res
        .status(200)
        .json({ result: "true", message: "Please check your email", data });
    } else {
      return res.status(400).json({
        result: "false",
  ...
  try {
    ...
    if (!status) {
      return res
        ...
        .json({
          result: "false",
          message: "required field is status (1 for innt and 2 for innntout)",
        });
    }
    if (status == "1") {
      const cat = await Maincategory.findOne({
      ...
      const cat = await Maincategory.findOne({
        maincategory_englishName: "InntOut",
      });
      const id = cat._id;
  ...
  try {
    ...
    if (!categoryId || !shopId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameter is categoryId,shopId",
        });
    }
...
const filterproducts = async (req, res) => {
  try {
    const { sub_subcategoryId, categoryId, price, rating, discount, price1 } =
      req.body;
    ...
    if (!sub_subcategoryId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are sub_subcategoryId and optional are (price, rating, discount,price1)",
        });
    }
    ...
    if (!status) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are status and optional are (price, rating, discount,categoryId,price1(1 for deals and 2 for newarrival))",
        });
    }
    if (status == 1) {
...
const tradingNowfilterproducts = async (req, res) => {
  try {
    const { productIds, categoryId, price, rating, discount, price1 } =
      req.body;
    ...
    if (!productIds) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are productIds and optional are (price, categoryId, rating, discount, price1)",
        });
    }

    // Convert each string in the productIdsArray to an ObjectId
    const productIdsObjectIds = productIds.map(
...
const searchProduct = async (req, res) => {
  try {
    ...
    if (!key) {
      res
        .status(400)
        .json({ result: "false", message: "required field is key" });
    }
    ...
    if (productList || productList.length > 0) {
      ...
      res
        ...
        .json({
          result: "true",
          message: "Products list get sucessfully",
          data: data,
        });
    }
  } catch (error) {
...
const inntRating = async (req, res) => {
  ...
  if (!userId || !rating) {
    return res
      ...
      .json({
        result: "false",
        message:
          "required parameters are userId,rating and comment is optional",
      });
  }
  ...
  try {
    ...
    if (data) {
      await Innt_rating.findOneAndUpdate(
        { userId },
        { $set: { rating, comment } },
        { new: true }
      );
...
const inntOutaddOrder = async (req, res) => {
  try {
    ...
    if (!userId || !checkoutId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are userId,checkoutId,note,addressId,payment_status,delivery_place,payType,paymentMode,grand_total",
        });
    }
    ...
    await Inntoutcart.findOneAndUpdate(
      { userId: userId, booking_status: 0 },
      { booking_status: 1 },
      { new: true }
    );
    await Checkout.findOneAndUpdate(
      { userId: userId, check_status: 0, status: 1 },
      updatedData,
      { new: true }
    );
    res
...
const InntorderList = async (req, res) => {
  try {
    ...
    const data = await Order.find({ userId, status: 1 })
      .populate("shopId")
      .sort({ _id: -1 });
    if (data) {
      res
...
const requestReturn_order = async (req, res) => {
  try {
    ...
    if (!userId || !productId || !orderId || !reason) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are productId,shopId,orderId,userId,reason",
        });
    }
    const insertData = new Returnorder({
      productId,
      shopId,
      orderId,
      userId,
    ...
    res
      ...
      .json({
        result: "false",
        message: "Order return request add successfully",
        data: data,
      });
  } catch (error) {
    // Handle error and return error response
    ...
    if (!userId || !otp || !mobile_number) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are {userId,otp,gender,user_profile,first_name,last_name,dob,email,mobile_number",
        });
    }
  ...
  try {
    ...
    if (!userId || !otp || !newEmail) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameters are userId,newEmail,otp",
        });
    }
    const matchData = await User.findOne({ _id: userId, otp: otp });
    if (!matchData) {
...
const sendGift = async (req, res) => {
  try {
    const { sender_userId, message, selectOne, name, contact_medium, amount } =
      req.body;
    if (!sender_userId) {
      return res
        .status(400)
        .json({
          result: "false",
          message:
            "Required fields are sender_userId,message,selectOne,name,contact_medium,amount",
        });
    }
    const validation = await User.findOne({ _id: sender_userId });
    if (
      validation.mobile_number == contact_medium ||
      validation.email == contact_medium
    ) {
      return res
        .status(400)
        .json({
    ...
    const matchData = await User.findOne({
      $or: [{ mobile_number: contact_medium }, { email: contact_medium }],
    });
    if (!matchData) {
      return res
        .status(400)
        .json({ result: "false", message: "User is not exixt in Innt" });
    }
    const id = matchData._id;
    const generated_code = generateRandomNumber();
    const insertData = new Gift({
      sender_userId,
      receiver_userId: id,
      message,
      selectOne,
    ...
    res
      ...
      .json({
        result: "true",
        message: "Data inserted sucessfullly",
        data: data,
      });
  } catch (err) {
    console.log(err.message);
  ...
  try {
    ...
    if (!sender_userId) {
      return res
        ...
        .json({
          result: "false",
          message: "Required fields are sender_userId",
        });
    }

    const matchData = await Gift.find({ sender_userId }).populate(
      "sender_userId"
    );
    if (!matchData) {
      return res
    ...
    res
      ...
      .json({
        result: "true",
        message: "Gift list got  sucessfullly",
        data: matchData,
      });
  } catch (err) {
    console.log(err.message);
...
const getGift_list = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ result: "false", message: "Required fields are userId" });
    }

    const matchData = await Gift.find({ receiver_userId: userId }).populate(
      "sender_userId"
    );
    if (!matchData) {
      return res
...
const improvementSuggestion = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ result: "false", message: "Required fields are userId,text" });
    }
  ...
  if (!userId || !status) {
    return res
      ...
      .json({
        result: "false",
        message:
          "Required parameters are userId and status(1 for l to h, 2 for h to l ,3 for most recent ,4 for least)",
      });
  }
  ...
  try {
    ...
    if (status === "1") {
      sortedData = mergedData
        .slice()
        .sort((a, b) => a.productId.sale_price - b.productId.sale_price);
    } else if (status === "2") {
      sortedData = mergedData
        .slice()
        .sort((a, b) => b.productId.sale_price - a.productId.sale_price);
    } else if (status === "3") {
      sortedData = mergedData
        .slice()
        .sort(
          (a, b) =>
            new Date(b.productId.createdAt) - new Date(a.productId.createdAt)
        );
    } else if (status === "4") {
      sortedData = mergedData
        .slice()
        .sort(
          (a, b) =>
            new Date(a.productId.createdAt) - new Date(b.productId.createdAt)
        );
    }
    ...
    res
      ...
      .json({
        result: "true",
        message: "Product liked data list sorted",
        data: sortedData,
      });
  } catch (err) {
    console.log(err);
...
const wishList_Productsearch = async (req, res) => {
  ...
  if (!userId || !key) {
    return res
      ...
      .json({
        result: "false",
        message: "Required parameters are userId and key",
      });
  }
...
const addReportOf_product = async (req, res) => {
  ...
  if (!userId) {
    return res
      ...
      .json({
        result: "false",
        message: "Required parameters are productId,title,text,userId,shopId",
      });
  }
  ...
  try {
    ...
    res
      ...
      .json({
        result: "true",
        message: "Product  report sent sucessfully",
        data: insertData,
      });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (!userId) {
      return res
        ...
        .json({
          result: "false",
          message: "Required parameters are userId,friendId,qrcode",
        });
    }
    ...
    const dataMatch = await User.findOne({
      _id: friendId,
      friend_qrcode: qrcode,
    });
    if (!dataMatch) {
      return res
        .status(400)
...
const friendList = async (req, res) => {
  ...
  if (!userId) {
    return res
      .status(400)
      .json({ result: "false", message: "Required parameter is userId" });
  }

  try {
    // Fetch friends where the user is `userId`
    const data1 = await Friend.find({ userId })
      .populate("friendId")
      .sort({ _id: -1 });

    // Fetch friends where the user is `friendId`
    const data2 = await Friend.find({ friendId: userId })
      .populate("userId")
      .sort({ _id: -1 });
    ...
    const uniqueMergedData = mergedData.filter(
      (item, index, self) =>
        ...
        self.findIndex(
          (t) =>
            t.userId._id.equals(item.userId._id) &&
            t.friendId._id.equals(item.friendId._id)
        )
    );
    ...
    if (uniqueMergedData.length === 0) {
      return res
        .status(404)
        .json({ result: "false", message: "User has no friends" });
    }
    ...
    res
      ...
      .json({
        result: "false",
        message: "An error occurred while fetching the friend list",
      });
  }
};
...
const getDataToQrcode = async (req, res) => {
  ...
  if (!qrcode) {
    return res
      .status(400)
      .json({ result: "false", message: "Required parameters are qrcode" });
  }

  try {

    const data = await User.findOne({
      $or: [{ friend_qrcode: qrcode }, { payment_qrcode: qrcode }],
    });
    if(data){
     return res
        .status(200)
        .json({
          result: "true",
          message: "Qrcode scanned  sucessfully",
          data: data,
        });
    }
   ...
   const findData = await Qrcode.findOne({qrcode});
   if(findData){
    const qrcodes={
      qrcode_name:findData.qrcode_name,
      qrcodeId:findData._id,
    }

   if(findData.type === "Specific product"){
   const productDetails = await Product.findOne({ _id: findData.productId });
   return res
   .status(200)
   .json({
     result: "true",
     message: "Vendor Product details QR code scanned successfully",
     data: productDetails,qrcodes
   });

  }
  const shopDetails = await Vender.findOne({ _id: findData.shopId});
  return res
   .status(200)
   .json({
     result: "true",
     message: "Vendor Shop details QR code scanned successfully",
     data: shopDetails,qrcodes
   });
  }else{
    return res
    .status(404)
    .json({ result: "false", message: "No details found for the provided QR code" });
...
const sendMoney_request = async (req, res) => {
  ...
  if (!qrcode || !senderId) {
    return res
      ...
      .json({
        result: "false",
        message: "Required parameters are qrcode,amount,senderId",
      });
  }
  ...
  try {
    ...
    res
      ...
      .json({
        result: "true",
        message: "Otp generates scanned  sucessfully",
        data: dinu,
      });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (!points || !senderId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are points,senderId,key(send email and mobile in key parameter)",
        });
    }
    ...
    if (data.loyalty_point < points) {
      return res
        ...
        .json({
          result: "false",
          message: "You have not sufficient loyalty points",
        });
    }

    const findreceiverData = await User.findOne({
      $or: [{ email: key }, { mobile_number: key }],
    });
    if (!findreceiverData) {
      return res
        .status(400)
        .json({
          result: "false",
          message: "This phone number and email not exist user",
        });
    }
    ...
    const findFriend = await Friend.findOne({
      $or: [
        { userId: senderId, friendId: _id },
        { userId: _id, friendId: senderId },
      ],
    });
    if (!findFriend) {
      return res
        ...
        .json({
          result: "false",
          message: "Sorry, you are not friend with this user",
        });
    }
    ...
    res
      .status(200)
      .json({ result: "true", message: "You request have done sucessfully" });
  } catch (err) {
    console.log(err);
    res.status(500).json({ result: "false", message: err.message });
  }
...
const addCashback = async (req, res) => {
  try {
    ...
    if (money < 1000) {
      return res
        .status(400)
        .json({ result: "false", message: "You have not sufficient Cashback" });
    }

    const otp = generate_otp();
    const dinu = await User.findOneAndUpdate(
      { _id: userId },
      { otp: otp },
      { new: true }
    );
    ...
    res
      ...
      .json({
        result: "true",
        message: "You request have done sucessfully",
        data: dt,
      });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (!userId) {
      return res
        ...
        .json({
          result: "false",
          message: "Required parameters are userId,otp",
        });
    }
    ...
    await User.findOneAndUpdate(
      { _id: userId },
      { wallet: wallet_money, loyalty_point: 0 },
      { new: true }
    );
    //transjection
    const transId = generateRandomNumber();
...
const user_queries = async (req, res) => {
  try {
    ...
    if (!userId || !message) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are userId,title,message,phone,email,lname,fname",
        });
    }

    const insertData = new userQuery({
      userId,
      title,
      message,
      phone,
    ...
    res
      ...
      .json({
        result: "true",
        message: "You request inserted sucessfully",
        data: data,
      });
  } catch (err) {
    console.log(err);
    ...
    if (!amount || !senderId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are amount,senderId,key(send email and mobile in key parameter)",
        });
    }
    ...
    if (!findreceiverData) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Sorry, This phone number or email not exist in innt application",
        });
    }
    const otp = generate_otp();
    await User.findOneAndUpdate({ _id: senderId }, { otp: otp }, { new: true });
...
const verificationsendMoney_otp = async (req, res) => {
  ...
  if (!senderId) {
    return res
      ...
      .json({
        result: "false",
        message: "Required parameters are senderId,receiverId and otp,amount ",
      });
  }
  ...
  try {
    ...
    if (!data || !findreceiverData) {
      return res
        .status(400)
        .json({ result: "false", message: "Incorrect otp" });
    }
    ...
    res
      .status(200)
      .json({ result: "true", message: "Request have done sucessfully" });
  } catch (err) {
    console.log(err);
    res.status(500).json({ result: "false", message: err.message });
  }
...
const transjection_history = async (req, res) => {
  try {
    ...
    const data = await Transjection.find({
      $or: [{ userId: userId }, { receiverId: userId }],
    })
      .populate("receiverId userId")
      .sort({ _id: -1 });
    if (!data) {
      return res
        .status(400)
  ...
  try {
    ...
    if (!latitude || !longitude) {
      return res
        ...
        .json({
          result: false,
          message: "Required parameters are missing: latitude, longitude",
        });
    }

    const Maincategories = await Maincategory.findOne({
      maincategory_englishName: "InntOut",
    });
...
const userSend_message = async (req, res) => {
  const { text, venderId, friendId, userId, adminId, driverId, send_status } =
    req.body;
  if (!send_status) {
    return res
      .status(400)
      .json({
        result: "false",
        message:
          "required parameter are venderId,driverId,adminId,friendId,userId,text,send_status(1 for vender 3 friend,2 admin and 4 driver user for 0)",
      });
  }
  ...
  try {
    if (!req.file) {
      ...
      res
        .status(200)
        .json({ result: "true", message: "Message sent successfully" });
    } else {
  ...
  if (!userId) {
    return res
      ...
      .json({
        result: "false",
        message:
          "required parameter are userId and optional are venderId and friendId",
      });
  }

  try {
    if (userId && venderId) {
      const messages = await Chat.find({ venderId, userId }).populate(
        "venderId"
      );
      if (messages.length > 0) {
        await Chat.updateMany(
          { venderId, userId, send_status: 0, status: 0 },
          { status: 1 },
          { new: true }
        );
...
const allChatList = async (req, res) => {
  ...
  try {
    ...
    const messages = await Chat.find({
      userId: userId,
      venderId: { $ne: null },
      $or: [{ send_status: 1 }, { send_status: 0 }],
    })
      .populate("venderId")
      .lean()
      .sort({ _id: -1 });
    ...
    if (finalMurged.length > 0) {
      res
        ...
        .json({
          result: "true",
          message: "All chat list retrieved successfully",
          finalMurged,
        });
    } else {
  ...
  try {
    ...
    if (filteredMessages.length > 0) {
      res
        ...
        .json({
          result: "true",
          message: "Friend chat list retrieved successfully",
          filteredMessages,
        });
    } else {
      res.status(404).json({ result: "false", message: "Data not found" });
...
const coupon_list = async (req, res) => {
  try {
    ...
    const couponList = await Promotion.find({
      act_status: 1,
    ...
    if (!couponList || couponList.length === 0) {
      return res
        .status(404)
        .json({ result: false, message: "No coupons found" });
    }
    res.status(200).json({
      result: true,
      message: "Shop list retrieved successfully",
...
const socicalMedia_login = async (req, res) => {
  try {
    ...
    if (!socical_media_id || !fcm_id) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameter are socical_media_id,first_name,last_name,email,mobile_number,fcm_id,type(1 for facebook,2 for google and 3 app)",
        });
    }
    ...
    if (validation) {
        ...
        const token = jwt.sign(
          {
            userId: validation._id || 0,
            email: validation.email || 0,
            validation: validation.mobile_number || 0,
          },
          process.env.ACCESS_TOKEN_SECURITY,
          { expiresIn: "730d" }
        );
...
const takeUserAddress = async (req, res) => {
  try {
    ...
    if (!userId || !checkoutId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "Required parameters are userId,addressId,checkoutId,delivery_place",
        });
    }
    ...
    if (addressId) {
      ...
      const findData = await Checkout.findOne({
        userId: userId,
        _id: checkoutId,
        check_status: 0,
      });
      ...
      shopList.forEach((shop) => {
        ...
        console.log("hfdjdfj", distance);
      });
      const updatePromises = shopDistances.map(
        async ({ shopId, shipping_charge }) => {
          await Checkout.findOneAndUpdate(
            { _id: checkoutId },
...
const filterShop = async (req, res) => {
  try {
    const { currentlat, currentlon, rating, distances1, distances2 } = req.body;
    if (!currentlat || !currentlon) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are currentlat,currentlon ,rating,distances1,distances2",
        });
    }
    ...
    if (!shopData || shopData.length === 0) {
      return res
        .status(400)
        .json({ result: "false", message: "Shop  not found" });
    }
    ...
    for (const shop of shopList) {
      ...
      const distance = parseFloat(
        calculateDistance(currentlat, currentlon, storeLat, storeLon).toFixed(1)
      );
      console.log(distances1, distances2);

      if (distance >= distances1 && distance <= distances2) {
        datas.push({
          shopId: shop._id,
          shop_name: shop.shop_name,
          logo: shop.shop_logo,
          ...
          lon: shop.location.coordinates[0],
          lat: shop.location.coordinates[1],
          average_ratings: ratingsMap.get(shop._id.toString()) || 0,
          distances: distance || 0,
        });
      }
    }
    ...
    const filteredData = datas.filter((item) => item.average_ratings >= rating);
...
const refferalHistory = async (req, res) => {
  try {
    ...
    const data = await Refferal.find({ friendId: userId })
      .populate("userId")
      .sort({ _id: -1 });
    if (data.length === 0) {
      return res
    ...
    const reorderAttributes = (product_variations, attributes) => {
      return product_variations.map((variation) => {
        const attr = attributes.find(
          (attr) =>
            attr.name.toLowerCase() === variation.attribute_name.toLowerCase()
        );
        if (attr) {
          const { value } = attr;
          ...
          const reorderedValues = [
            value,
            ...attribute_values.filter((val) => val !== value),
          ];
    ...
    cart_data.forEach((cart) => {
      cart.products.forEach((product) => {
        if (product.productId.product_variation && product.attributes) {
          product.productId.product_variation = reorderAttributes(
            product.productId.product_variation,
            product.attributes
          );
        }
      });
  ...
  try {
    ...
    if (!userId || !addressId) {
      return res
        ...
        .json({
          result: "false",
          message: "Required parameters are userId and addressId",
        });
    }
  ...
  try {
    ...
    if (!userId) {
      return res
        ...
        .json({
          result: "false",
          message: "required parameters is userId,reason,text",
        });
    }
  ...
  try {
    ...
    if (!subSubcategoryId) {
      return res
        ...
        .json({
          result: "false",
          message: "Required parameter are subSubcategoryId ",
        });
    }
...
const usedGiftandPromotion_code = async (req, res) => {
  try {
    const { userId, code } = req.body;
    if (!userId || !code) {
      return res
        .status(400)
        .json({ result: "false", message: "Required fields are userId,code" });
    }

    const findData = await Gift.findOne({
      receiver_userId: userId,
      card_no: code,
    });
    if (!findData) {
      const findData1 = await Promotion.findOne({ cupon_code: code });
      if (!findData1) {
        return res
          .status(400)
          .json({ result: "false", message: "Invalid code number" });
      }
      res
        .status(200)
        .json({
          result: "true",
          message: " Promotion ode applied sucessfully",
          data: findData1,
        });
    } else {
      await Gift.findOneAndUpdate(
        { receiver_userId: userId, card_no: code },
        { status: 1 },
        { new: true }
      );
      res
        ...
        .json({
          result: "true",
          message: "Gift code applied sucessfully",
          data: findData,
        });
    }
  } catch (err) {
...
const favouriteShopList_filter = async (req, res) => {
  try {
    const { userId, lat, lon, distances1, distances2, rating } = req.body;
    if (!userId || !lat) {
      return res
        .status(400)
        .json({
          result: "false",
          message:
            "required parameter are userId,lat,lon,distances1,distances2,rating",
        });
    }
    const shoplist = await Shopfavourite.find({
      userId: userId,
    ...
    for (const shop of shoplist) {
      ...
      const storeLon = shop.shopId.location.coordinates[0];
      const distance = parseFloat(
        calculateDistance(lat, lon, storeLat, storeLon).toFixed(1)
      );
      console.log(distances1, distances2);
      if (distance >= distances1 && distance <= distances2) {
        data.push({
...
const addReportOf_shop = async (req, res) => {
  const { text, title, userId, shopId, status } = req.body;
  if (!userId) {
    return res
      .status(400)
      .json({
        result: "false",
        message:
          "Required parameters are title,text,userId,shopId,status(0 for innt and 1 for inntOut)",
      });
  }

  try {
    const data = new shopReport({ status, text, userId, title, shopId });
    const insertData = await data.save();
    res
      .status(200)
      .json({
        result: "true",
        message: "Shop  report sent sucessfully",
        data: insertData,
      });
  } catch (err) {
    console.log(err);
...
const inntoutProductFilter = async (req, res) => {
  try {
    const {
      categoryId,
      lat,
    ...
    if (!categoryId) {
      return res
        ...
        .json({
          result: "false",
          message:
            "required parameters are categoryId,distances1,distances2,price1,price2, rating,discount,lat,lon,",
        });
    }


    //find the shopdistance
    const productData = await Product.find({categoryId}).populate('venderId');
	if(!productData || productData.length===0){
		return res
        .status(400)
         .json({ result: "false", message: "Product data not found" });
	 ...
	 const data = [];
	 for (const shop of productData) {
	   const storeLat = shop.venderId.location.coordinates[1];
	   const storeLon = shop.venderId.location.coordinates[0];
	   const distance = parseFloat(
		 calculateDistance(lat, lon, storeLat, storeLon).toFixed(1)
	   );
     ...
     if(distance >= distances1 && distance <= distances2){
      ...
      discount: shop.discount[0].discount_value,
      discount_name: shop.discount[0].discount_type,
      rating: shop.rating,
      distance:distance || 0
      })
     }
...
console.log(data)
  ...
  var filteredData ;
    if (price && price2) {
      filteredData = data.filter(item => item.sale_price >= price && item.sale_price <= price2);
    }
    
    let filters;
    if (rating) {
      filters = filteredData.filter(item => item.rating >= rating);
    }
    
   let lastFilter;
    if (discount) {
      lastFilter = filters.filter(item => item.discount_name === "Percent" && item.discount <= discount || item.discount_name === "Unit");
    }

    if (!lastFilter || lastFilter.length === 0) {
      return res.status(400).json({ result: "false", message: "Filtered product data not found" });
    }
...
module.exports = {
  ...
  usedGiftandPromotion_code,
  favouriteShopList_filter,
  addReportOf_shop,
  inntoutProductFilter,
};
